...
const FilterByTags = ({onChange, save, allTags, defaultValue=null}: any) => {
  ...
  const resetTagsList = () => {
    ...
    ? JSON.parse(JSON.stringify({...defaultValue, filterTags: defaultValue.filterTags.filter(({type}: any) => type !== 'radio')})) 
    : {
      searchText: '',
      datasetType: '',
  ...
  const changeValue = (arg: React.SetStateAction<{ searchText?: string; datasetType?: string; filterTags?: never[] }>) => {
  ...
  return <>
    ...
    <SelectTags filterTags={filterParmas.filterTags} setFilterTags={(tags: any) => changeValue({filterTags: tags})} />
